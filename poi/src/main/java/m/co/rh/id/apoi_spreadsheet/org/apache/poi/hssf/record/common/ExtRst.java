...
public class ExtRst implements Comparable<ExtRst>, GenericRecord {
    ...
    protected ExtRst(LittleEndianInput in, int expectedLength) {
        ...
        if(reserved != 1) {
            Log.w(TAG, String.format("ExtRst has wrong magic marker, expecting 1 but found %s - ignoring", reserved));
        ...
        if(extraDataLength < 0) {
            Log.w(TAG, String.format("ExtRst overran by %s bytes", -extraDataLength));
            extraDataLength = 0;
        }
