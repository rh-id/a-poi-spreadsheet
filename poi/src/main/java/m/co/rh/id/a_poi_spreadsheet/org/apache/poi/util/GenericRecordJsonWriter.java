...
import m.co.rh.id.a_poi_spreadsheet.org.apache.poi.util.GenericRecordUtil.AnnotatedFlag;
...
public class GenericRecordJsonWriter implements Closeable {
    ...
    static {
        ...
        handler(Point.class, GenericRecordJsonWriter::printPoint);
        handler(Dimension2D.class, GenericRecordJsonWriter::printDimension);
        handler(Rect.class, GenericRecordJsonWriter::printRectangle);
        handler(Path.class, GenericRecordJsonWriter::printPath);
        handler(Matrix.class, GenericRecordJsonWriter::printAffineTransform);
        handler(Color.class, GenericRecordJsonWriter::printColor);
        handler(Bitmap.class, GenericRecordJsonWriter::printImage);
    ...
    protected boolean printRectangle(String name, Object o) {
        printName(name);
        Rect p = (Rect) o;
        fw.write("{ \"x\": " + p.left + ", \"y\": " + p.top + ", \"width\": " + p.width() + ", \"height\": " + p.height() + " }");
        return true;
    }
        ...
        while (!iter.hasNext()) {
            ...
            switch (segType) {
                case Move:
                    ...
                    fw.write("\"move\", \"x\": " + movePts.x + ", \"y\": " + movePts.y);
                    break;
                case Line:
                    PointF linePts = pathSegment.getPoints()[1];
                    fw.write("\"lineto\", \"x\": " + linePts.x + ", \"y\": " + linePts.y);
                    break;
                case Quadratic:
                    PointF qControlPts = pathSegment.getPoints()[1];
                    PointF qEndPts = pathSegment.getPoints()[2];
                    fw.write("\"quad\", \"x1\": " + qControlPts.x + ", \"y1\": " + qControlPts.y + ", \"x2\": " + qEndPts.x + ", \"y2\": " + qEndPts.y);
                    break;
                case Cubic:
                    ...
                    fw.write("\"cubic\", \"x1\": " + cFirstCtrlPts.x + ", \"y1\": " + cFirstCtrlPts.y + ", \"x2\": " + cSecondCtrlPts.x + ", \"y2\": " + cSecondCtrlPts.y + ", \"x3\": " + cEndCtrlPts.x + ", \"y3\": " + cEndCtrlPts.y);
                    break;
                case Close:
                    fw.write("\"close\"");
                    break;
