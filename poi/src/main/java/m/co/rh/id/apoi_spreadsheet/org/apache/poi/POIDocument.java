...
import m.co.rh.id.apoi_spreadsheet.org.apache.poi.hpsf.PropertySet;
import m.co.rh.id.apoi_spreadsheet.org.apache.poi.hpsf.PropertySetFactory;
import m.co.rh.id.apoi_spreadsheet.org.apache.poi.hpsf.SummaryInformation;
import m.co.rh.id.apoi_spreadsheet.org.apache.poi.hpsf.WritingNotSupportedException;
...
import m.co.rh.id.apoi_spreadsheet.org.apache.poi.poifs.crypt.cryptoapi.CryptoAPIDecryptor;
import m.co.rh.id.apoi_spreadsheet.org.apache.poi.poifs.crypt.cryptoapi.CryptoAPIEncryptor;
    ...
    private <T> T readPropertySet(Class<T> clazz, String name, boolean warnIfNull) {
        ...
        try {
            ...
            if (clazz.isInstance(ps)) {
                ...
                if (warnIfNull) {
                    Log.w(TAG, String.format("%s property set came back as null", localName));
                }
            }
        } catch (IOException e) {
            Log.e(TAG, "can't retrieve property set", e);
        }
        return null;
    }
