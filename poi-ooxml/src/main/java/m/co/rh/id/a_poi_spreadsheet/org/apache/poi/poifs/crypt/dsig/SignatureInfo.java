...
import static m.co.rh.id.a_poi_spreadsheet.org.apache.poi.poifs.crypt.dsig.facets.SignatureFacet.XML_DIGSIG_NS;
...
import m.co.rh.id.a_poi_spreadsheet.org.apache.poi.ooxml.util.DocumentHelper;
...
import m.co.rh.id.a_poi_spreadsheet.org.apache.poi.poifs.crypt.dsig.services.RelationshipTransformService;
...
public class SignatureInfo {
    ...
    private static final String TAG = "SignatureInfo";
    ...
    private final class SignaturePartIterator implements Iterator<SignaturePart> {
        ...
        public boolean hasNext() {
            while (sigRels == null || !sigRels.hasNext()) {
                ...
                Log.d(TAG, String.format("Digital Signature Origin part: %s", sigPart));
                try {
                    sigRels = sigPart.getRelationshipsByType(PackageRelationshipTypes.DIGITAL_SIGNATURE).iterator();
                } catch (InvalidFormatException e) {
                    Log.w(TAG, "Reference to signature is invalid.");
                }
            }
            return true;
        }
        ...
        public SignaturePart next() {
            ...
            do {
                try {
                    ...
                    Log.d(TAG, String.format("XML Signature part: %s", sigRelPart));
                } catch (InvalidFormatException e) {
                    Log.w(TAG, "Reference to signature is invalid.");
                }
    ...
    public DOMSignedInfo preSign(final DOMSignContext xmlSignContext)
        ...
        for (SignatureFacet signatureFacet : signatureConfig.getSignatureFacets()) {
            Log.d(TAG, String.format("invoking signature facet: %s", signatureFacet.getClass().getSimpleName()));
        ...
        for (XMLObject object : objects) {
            Log.d(TAG, String.format("object java type: %s", object.getClass().getName()));
            List<XMLStructure> objectContentList = object.getContent();
            for (XMLStructure objectContent : objectContentList) {
                Log.d(TAG, String.format("object content java type: %s", objectContent.getClass().getName()));
    ...
    protected void writeDocument(Document document) throws MarshalException {
        ...
        Log.d(TAG, "output signed Office OpenXML document");
    ...
    private Element getDsigElement(final Document document, final String localName) {
        ...
        Log.w(TAG, String.format("Signature element '%s' was %s", localName, sigValNl.getLength() == 0 ? "not found" : "multiple times"));
    ...
    private static final class XmlProviderInitSingleton {
        ...
        private Provider getProvider(String className) {
            try {
                ...
                Log.d(TAG, String.format("XMLDsig-Provider '%s' can't be found - trying next.", className));
                return null;
            }
        }
