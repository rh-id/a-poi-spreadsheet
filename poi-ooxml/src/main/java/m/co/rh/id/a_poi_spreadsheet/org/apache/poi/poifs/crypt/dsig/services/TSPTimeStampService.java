...
package m.co.rh.id.a_poi_spreadsheet.org.apache.poi.poifs.crypt.dsig.services;
...
import m.co.rh.id.a_poi_spreadsheet.org.apache.poi.poifs.crypt.dsig.SignatureConfig.CRLEntry;
import m.co.rh.id.a_poi_spreadsheet.org.apache.poi.poifs.crypt.dsig.SignatureInfo;
import m.co.rh.id.a_poi_spreadsheet.org.apache.poi.poifs.crypt.dsig.services.TimeStampHttpClient.TimeStampHttpClientResponse;
...
public class TSPTimeStampService implements TimeStampService {
    ...
    private static final String TAG = "TSPTimeStampService";
    ...
    public byte[] timeStamp(SignatureInfo signatureInfo, byte[] data, RevocationData revocationData) throws Exception {
        ...
        if (0 != timeStampResponse.getStatus()) {
            Log.d(TAG, String.format("status: %d", timeStampResponse.getStatus()));
            Log.d(TAG, String.format("status string: %s", timeStampResponse.getStatusString()));
            PKIFailureInfo failInfo = timeStampResponse.getFailInfo();
            if (null != failInfo) {
                Log.d(TAG, String.format("fail info int value: %d", failInfo.intValue()));
                if (/*PKIFailureInfo.unacceptedPolicy*/(1 << 8) == failInfo.intValue()) {
                    Log.d(TAG, "unaccepted policy");
                }
            }
        ...
        Log.d(TAG, String.format("signer cert serial number: %s", signerCertSerialNumber));
        Log.d(TAG, String.format("signer cert issuer: %s", signerCertIssuer));
        ...
        Log.d(TAG, String.format("time-stamp token time: %s", timeStampToken.getTimeStampInfo().getGenTime()));
    ...
    protected CRLEntry downloadCRL(SignatureConfig signatureConfig, String url) {
        ...
        try {
            ...
            Log.w(TAG, String.format("CRL download failed from %s", url), e);
            return null;
        }
    }
}