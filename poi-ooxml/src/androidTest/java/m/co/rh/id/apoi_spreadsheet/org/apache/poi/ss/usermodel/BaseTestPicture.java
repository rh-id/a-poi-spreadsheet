...
import m.co.rh.id.apoi_spreadsheet.base.model.Dimension;
...
public abstract class BaseTestPicture {
    ...
    public void resize() throws IOException {
        ...
        try (Workbook wb = _testDataProvider.openSampleWorkbook(fileName)) {
            ...
            assertEquals("the image height differs",
                    inpDim.getHeight(), cmpDim.getHeight(), emuPX * 6);
            assertEquals("the image width differs",
                    inpDim.getWidth(), cmpDim.getWidth(), emuPX * 6);
            assertEquals("the starting column differs",
                    inpCA.getCol1(), cmpCA.getCol1());
            assertEquals("the column x-offset differs",
                    inpCA.getDx1(), cmpCA.getDx1(), 1);
            assertEquals("the column y-offset differs - image has moved",
                    inpCA.getDy1(), origDy1, 1);
            assertEquals("the column x-offset differs - image has moved",
                    inpCA.getDx1(), origDx1, 1);
            assertEquals("the column y-offset differs",
                    inpCA.getDy1(), cmpCA.getDy1(), 1);
            assertEquals("the ending columns differs",
                    inpCA.getCol2(), cmpCA.getCol2());
            ...
            assertEquals("the image height differs",
                    imgDim.getHeight(), inpDim.getHeight() / emuPX, 1);
            assertEquals("the image width differs",
                    imgDim.getWidth(), inpDim.getWidth() / emuPX, 1);
        }
    }
