...
//        }
    ...
    private void checkWorkbookGrouping(Workbook wb, Boolean[] collapsed, boolean[] hidden, int[] outlineLevel) {
        ...
        for (int i = sheet.getFirstRowNum(); i < sheet.getLastRowNum(); i++) {
            ...
            assertNotNull("Could not read row " + i,
                    row);
            assertNotNull("Could not read row " + i,
                    row.getCTRow());
            assertEquals("Row: " + i + ": collapsed",
                    collapsed[i - sheet.getFirstRowNum()], row.getCTRow().getCollapsed());
            assertEquals("Row: " + i + ": hidden",
                    hidden[i - sheet.getFirstRowNum()], row.getCTRow().getHidden());

            assertEquals("Row: " + i + ": level",
                    outlineLevel[i - sheet.getFirstRowNum()], row.getCTRow().getOutlineLevel());
        }
