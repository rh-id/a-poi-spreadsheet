...
public abstract class BaseTestBugzillaIssues {
    private static final String TAG = "BaseTestBugzillaIssues";
    ...
    private static void assertAlmostEquals(double expected, double actual, float factor) {
        ...
        assertTrue(actual + " not within " + fuzz + " of " + expected,
                diff <= fuzz);
    }
    ...
    /* FIXME: Autosize depends on SheetUtil.canComputeColumnWidth which is still not know how to implement
    @Test
...
//        }
    ...
    private void assertGreaterThan(String message, double a, double b) {
        assertTrue(message + ": " + "Expected: " + a + " > " + b,
                a > b);
    }
    ...
    public void bug46670() throws IOException {
        try (Workbook wb1 = _testDataProvider.createWorkbook()) {
            ...
            try (Workbook wb2 = _testDataProvider.writeOutAndReadBack(wb1)) {
                ...
                assertThrows("Shouldn't be able to evaluate without the other file",
                        Exception.class, () -> evaluateCell(wb2, c1));
                assertThrows("Shouldn't be able to evaluate without the other file",
                        Exception.class, () -> evaluateCell(wb2, c2));
    ...
    public void bug58260() throws IOException {
        ...
        try (Workbook wb = _testDataProvider.createWorkbook()) {
            ...
            assertThrows("Should fail after " + maxStyles + " styles, but did not fail",
                    IllegalStateException.class, wb::createCellStyle);
    ...
    public void test52684() throws IOException {
        try (Workbook wb = _testDataProvider.createWorkbook()) {
            ...
            /*
             TODO this is asserted by original POI dev, it passed? but why?
             Possibly ThreadLocalUtil issue? as this test is run with POIJUnit4ClassRunner
             which has one thread only
             //assertEquals("12-312-345-123", formatter.formatCellValue(cell));
             */
            // Formatted values should be as per expected below,
            // following the cell format set above 000-00000-000
            assertEquals("123-12345-123", formatter.formatCellValue(cell));
        }
    }
    ...
    public void test58896() throws IOException {
        ...
        try (Workbook wb = _testDataProvider.createWorkbook(nrows + 1)) {
            ...
            Log.d(TAG, String.format("%s column autosizing timing...", wb.getClass().getName()));
            ...
            Log.d(TAG, String.format("Populate sheet time: %f ms (%f ns/cell)", populateSheetTime, populateSheetTimePerCell_ns));

            Log.d(TAG, "Autosizing...");
            final long t1 = time();
            for (int c = 0; c < ncols; c++) {
                final long t2 = time();
                sh.autoSizeColumn(c);
                Log.d(TAG, String.format("Column %d took %f ms", c, delta(t2)));
            }
            ...
            Log.d(TAG, String.format("Auto sizing columns took a total of %f ms (%f ms per column)", autoSizeColumnsTime, autoSizeColumnsTimePerColumn));
            Log.d(TAG, String.format("Best fit width time per cell: %f ns", bestFitWidthTimePerCell_ns));

            final double totalTime_s = (populateSheetTime + autoSizeColumnsTime) / 1000;
            Log.d(TAG, String.format("Total time: %f s", totalTime_s));
        }
    ...
    private void checkFormulaPreevaluatedString(Workbook readFile) {
        ...
        for (Cell cell : row) {
            ...
            assertTrue("unexpected cell type", ctype == CellType.STRING || ctype == CellType.FORMULA);
