...
import m.co.rh.id.apoi_spreadsheet.org.apache.poi.xssf.streaming.SXSSFFormulaEvaluator.RowFlushedException;
import m.co.rh.id.apoi_spreadsheet.org.apache.poi.xssf.streaming.SXSSFFormulaEvaluator.SheetsFlushedException;
...
public final class TestSXSSFFormulaEvaluation extends BaseTestFormulaEvaluator {
    ...
    public void testEvaluateAllFails() throws IOException {
        try (SXSSFWorkbook wb = new SXSSFWorkbook(5)) {
            ...
            assertThrows("Evaluate All shouldn't work, as some cells outside the window",
                    RowFlushedException.class, eval::evaluateAll);
        }
        ...
        try (XSSFWorkbook xwb = new XSSFWorkbook()) {
            ...
            try (SXSSFWorkbook wb = new SXSSFWorkbook(xwb, 5)) {
                ...
                FormulaEvaluator eval = wb.getCreationHelper().createFormulaEvaluator();
                assertThrows("Evaluate All shouldn't work, as sheets flushed",
                        SheetsFlushedException.class, eval::evaluateAll);
            }
        }
    }

    @Test
    public void testLogSpam() throws IOException {
        try (SXSSFWorkbook wb = new SXSSFWorkbook(5)) {
            SXSSFSheet s = wb.createSheet();
    ...
    public void testEvaluateRefOutsideWindowFails() throws IOException {
        try (SXSSFWorkbook wb = new SXSSFWorkbook(5)) {
            ...
            FormulaEvaluator eval = wb.getCreationHelper().createFormulaEvaluator();
            assertThrows("Evaluate shouldn't work, as reference outside the window",
                    RowFlushedException.class, () -> eval.evaluateFormulaCell(c));
        }
    }
