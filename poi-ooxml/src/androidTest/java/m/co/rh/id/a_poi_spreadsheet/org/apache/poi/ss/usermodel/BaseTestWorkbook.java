...
package m.co.rh.id.a_poi_spreadsheet.org.apache.poi.ss.usermodel;
...
import m.co.rh.id.a_poi_spreadsheet.org.apache.poi.ss.ITestDataProvider;
...
public abstract class BaseTestWorkbook {
    ...
    public void createSheet() throws IOException {
        try (Workbook wb = _testDataProvider.createWorkbook()) {
            ...
            ex = assertThrows("should have thrown exception due to duplicate sheet name",
                    IllegalArgumentException.class,
            ...
            for (String sheetName : invalidNames) {
                assertThrows("should have thrown exception due to invalid sheet name: " + sheetName,
                        IllegalArgumentException.class,
    ...
    protected void assertSheetOrder(Workbook wb, String... sheets) {
        ...
        assertEquals("Had: " + sheetNames, sheets.length, wb.getNumberOfSheets());
        for (int i = 0; i < wb.getNumberOfSheets(); i++) {
            assertEquals("Had: " + sheetNames, sheets[i], wb.getSheetAt(i).getSheetName());
        }
    }
    ...
    protected static void assertCloseDoesNotModifyFile(String filename, Workbook wb) throws IOException {
        ...
        assertArrayEquals(filename + " sample file was modified as a result of closing the workbook", before, after);
    }
    ...
    @Test
    public void testSheetNameTrimming() throws IOException {
        try (Workbook workbook = _testDataProvider.createWorkbook()) {
