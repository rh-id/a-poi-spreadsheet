...
public abstract class BaseTestSheetShiftRows {
    ...
    public void testShiftRowBreaks() throws IOException {
        ...
        try (Workbook wb = _testDataProvider.createWorkbook()) {
            ...
            s.shiftRows(4, 4, 2);
            assertTrue("Row number 6 should have a pagebreak",
                    s.isRowBroken(6));
        }
    }
    ...
    public void testBug46742_52903_shiftHyperlinks() throws IOException {
        try (Workbook wb = _testDataProvider.createWorkbook()) {
            ...
            try (Workbook read = _testDataProvider.writeOutAndReadBack(wb)) {
                ...
                assertNull("Document hyperlink should be moved, not copied",
                        sh.getHyperlink(0, 0));
                assertNull("URL hyperlink should be moved, not copied",
                        sh.getHyperlink(0, 1));
                ...
                for (Hyperlink link : sh.getHyperlinkList()) {
                    ...
                    assertNotEquals("Row 4, including the hyperlink at C4, should have " +
                                    "been deleted when Row 1 was shifted on top of it.",
                            linkAddress, unexpectedLinkAddress);
                }
    ...
    @Test
    public void checkMergedRegions56454() throws IOException {
        try (Workbook wb = _testDataProvider.createWorkbook()) {
            Sheet sheet = wb.createSheet();
