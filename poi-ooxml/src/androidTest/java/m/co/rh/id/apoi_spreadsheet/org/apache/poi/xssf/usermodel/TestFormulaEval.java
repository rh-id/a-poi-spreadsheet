...
public class TestFormulaEval {
    @Test
    public void testCircularRef() throws IOException {
    ...
    @Test
    public void testCircularRef2() throws IOException {
        try (Workbook wb = new XSSFWorkbook()) {
            Sheet sheet = wb.createSheet();
            Row row = sheet.createRow(0);
    ...
    @Test
    public void testExceptionForWrongFormula1() throws IOException {
        try (Workbook wb = new XSSFWorkbook()) {
            ...
            try {
                ...
                assertTrue("Had: " + e.getMessage(), e.getMessage().contains("test-sheet"));
                assertTrue("Had: " + e.getMessage(), e.getMessage().contains("Sheet123"));
                assertTrue("Had: " + e.getMessage(), e.getMessage().contains("R6C13"));
            }
        }
    }

    @Test
    public void testExceptionForWrongFormula2() throws IOException {
        try (Workbook wb = new XSSFWorkbook()) {
            Sheet sheet = wb.createSheet("test-sheet");
            Row row = sheet.createRow(0);
            Cell cell0 = row.createCell(0);
            ...
            try {
                ...
                assertTrue("Had: " + e.getMessage(), e.getMessage().contains("Sheet123"));
                assertTrue("Had: " + e.getMessage(), e.getMessage().contains("R6C13"));
                assertTrue("Had: " + e.getMessage(), e.getMessage().contains("SUM"));
            }
        }
    }
}
