...
public abstract class BaseTestFont {
     ...
     * String defaultName, short defaultSize, short defaultColor
     */
    ...
    @Test
    public final void baseTestDefaultFont() throws IOException {
        for (Object[] args : defaultFont()) {
            String defaultName = (String) args[0];
            short defaultSize = (short) args[1];
            short defaultColor = (short) args[2];
    ...
    public void testRGBColor() throws Exception {
        try (Workbook wb1 = _testDataProvider.createWorkbook()) {
            byte[] colorHex = new byte[]{(byte) 0xFF, (byte) 0xEB, (byte) 0x85};
            ExtendedColor color = wb1.getCreationHelper().createExtendedColor();
            color.setRGB(colorHex);
            assertArrayEquals(colorHex, color.getRGB());
        }
    }
}
