...
import m.co.rh.id.apoi_spreadsheet.org.apache.poi.ss.usermodel.WorkbookProvider;
...
import m.co.rh.id.apoi_spreadsheet.org.apache.poi.xssf.usermodel.helpers.ColumnHelper;
...
public final class TestXSSFSheet extends BaseTestXSheet {
    ...
    @Test
    public void headerFooterWithFormat() throws IOException {
    ...
    @Test
    public void testHeaderWithAmpersand() throws IOException {
    ...
    @Test
    public void createFreezePane_XSSF_withBug66173() throws IOException {
        try (XSSFWorkbook workbook = new XSSFWorkbook()) {
            XSSFSheet sheet = workbook.createSheet();
    ...
    @Test
    public void defaultActivePaneType() throws IOException {
        try (XSSFWorkbook workbook = new XSSFWorkbook()) {
            XSSFSheet sheet = workbook.createSheet();
    ...
    public void removeMergedRegion_lowlevel() throws IOException {
        try (XSSFWorkbook workbook = new XSSFWorkbook()) {
            ...
            assertNull("CTMergeCells should be deleted after removing the last merged region on the sheet.",
    ...
    @Test
    public void saveGroupColumns() throws IOException {
        try (
    ...
    @Test
    public void collapseGroupColumn() throws IOException {
        try (XSSFWorkbook workbook = new XSSFWorkbook()) {
            XSSFSheet sheet = workbook.createSheet();
            XSSFRow row0 = sheet.createRow(0);
            XSSFRow row1 = sheet.createRow(1);
    ...
    @Test
    public void expandGroupColumn() throws IOException {
        try (XSSFWorkbook workbook = new XSSFWorkbook()) {
            XSSFSheet sheet = workbook.createSheet();
            XSSFRow row0 = sheet.createRow(0);
            XSSFRow row1 = sheet.createRow(1);
    ...
    @Test
    public void collapseNonExistentGroupColumn() throws IOException {
        try (XSSFWorkbook workbook = new XSSFWorkbook()) {
            XSSFSheet sheet = workbook.createSheet();
            XSSFRow row0 = sheet.createRow(0);
            XSSFRow row1 = sheet.createRow(1);
    ...
    private static void checkColumnGroup(
        ...
        assertEquals("from column index",
                fromColumnIndex, col.getMin() - 1); // 1 based
        assertEquals("to column index",
                toColumnIndex, col.getMax() - 1); // 1 based
        assertEquals("isSetHidden",
                isSetHidden, col.isSetHidden());
        assertEquals("isSetCollapsed",
                isSetCollapsed, col.isSetCollapsed()); //not necessarily set
    }
    ...
    private static void checkColumnGroupIsCollapsed(CTCol col, int fromColumnIndex, int toColumnIndex,
        ...
        assertEquals("to column index",
                toColumnIndex, col.getMax() - 1); // 1 based
        assertTrue("isSetHidden",
                col.isSetHidden());
        assertEquals("isSetCollapsed",
                isSetCollapsed, col.isSetCollapsed());
        if (isSetCollapsed) {
            assertEquals("getCollapsed",
    ...
    private static void checkColumnGroupIsExpanded(CTCol col, int fromColumnIndex, int toColumnIndex) {
        ...
        assertFalse("isSetHidden",
                col.isSetHidden());
        assertTrue("isSetCollapsed",
                !col.isSetCollapsed() || !col.getCollapsed());
    }
    ...
    public void protectSheet_lowlevel() throws IOException {
        try (XSSFWorkbook wb = new XSSFWorkbook()) {
            ...
            assertNull("CTSheetProtection should be null by default",
                    pr);
            String password = "Test";
            sheet.protectSheet(password);
            pr = sheet.getCTWorksheet().getSheetProtection();
            assertNotNull("CTSheetProtection should be not null",
                    pr);
            assertTrue("sheet protection should be on",
                    pr.isSetSheet());
            assertTrue("object protection should be on",
                    pr.isSetObjects());
            assertTrue("scenario protection should be on",
                    pr.isSetScenarios());
    ...
    public void bug54607() throws IOException {
        ...
        args.add(new String[]{"54607.xlsx", "1, 0, 0", "0, 0, 0"});
        // run with some other flie to see
        args.add(new String[]{"54436.xlsx", "0", "0"});
        args.add(new String[]{"TwoSheetsNoneHidden.xlsx", "0, 0", "0, 0"});
        args.add(new String[]{"TwoSheetsNoneHidden.xls", "0, 0", "0, 0"});
        for (String[] arg : args) {
            String file = arg[0];
            String topRows = arg[1];
            String leftCols = arg[2];
    ...
    public void bug55745() throws Exception {
        try (XSSFWorkbook wb = XSSFTestDataSamples.openSampleWorkbook("55745.xlsx")) {
            ...
            assertEquals("Sheet should contain 8 tables",
                    8, tables.size());
    ...
    public void testCopyOneRow() throws IOException {
        ...
        try (final XSSFWorkbook wb = XSSFTestDataSamples.openSampleWorkbook("XSSFSheet.copyRows.xlsx")) {
            ...
            // Style
            cell = CellUtil.getCell(destRow, col++);
            assertEquals("[Style] B7 cell value",
                    "Red", cell.getStringCellValue());
            assertEquals("[Style] B7 cell style",
                    CellUtil.getCell(srcRow, 1).getCellStyle(), cell.getCellStyle());

            // Blank
            cell = CellUtil.getCell(destRow, col++);
            assertEquals("[Blank] C7 cell type",
                    CellType.BLANK, cell.getCellType());

            // Error
            cell = CellUtil.getCell(destRow, col++);
            assertEquals("[Error] D7 cell type",
                    CellType.ERROR, cell.getCellType());
            final FormulaError error = FormulaError.forInt(cell.getErrorCellValue());
            //FIXME: XSSFCell and HSSFCell expose different interfaces. getErrorCellString would be helpful here
            assertEquals("[Error] D7 cell value",
                    FormulaError.NA, error);

            // Date
            cell = CellUtil.getCell(destRow, col++);
            assertEquals("[Date] E7 cell type",
                    CellType.NUMERIC, cell.getCellType());
            final Date date = LocaleUtil.getLocaleCalendar(2000, Calendar.JANUARY, 1).getTime();
            assertEquals("[Date] E7 cell value",
                    date, cell.getDateCellValue());

            // Boolean
            cell = CellUtil.getCell(destRow, col++);
            assertEquals("[Boolean] F7 cell type",
                    CellType.BOOLEAN, cell.getCellType());
            assertTrue("[Boolean] F7 cell value",
                    cell.getBooleanCellValue());

            // String
            cell = CellUtil.getCell(destRow, col++);
            assertEquals("[String] G7 cell type",
                    CellType.STRING, cell.getCellType());
            assertEquals("[String] G7 cell value",
                    "Hello", cell.getStringCellValue());

            // Int
            cell = CellUtil.getCell(destRow, col++);
            assertEquals("[Int] H7 cell type",
                    CellType.NUMERIC, cell.getCellType());
            assertEquals("[Int] H7 cell value",
                    15, (int) cell.getNumericCellValue());

            // Float
            cell = CellUtil.getCell(destRow, col++);
            assertEquals("[Float] I7 cell type",
                    CellType.NUMERIC, cell.getCellType());
            assertEquals("[Float] I7 cell value",
                    12.5, cell.getNumericCellValue(), FLOAT_PRECISION);

            // Cell Formula
            cell = CellUtil.getCell(destRow, col++);
            assertEquals("Sheet1!J7", new CellReference(cell).formatAsString());
            assertEquals("[Cell Formula] J7 cell type",
                    CellType.FORMULA, cell.getCellType());
            assertEquals("[Cell Formula] J7 cell formula",
                    "5+2", cell.getCellFormula());
            ...
            // Data Format
            cell = CellUtil.getCell(destRow, col++);
            assertEquals("[Data Format] O7 cell type",
                    CellType.NUMERIC, cell.getCellType());
            assertEquals("[Data Format] O7 cell value",
                    100.20, cell.getNumericCellValue(), FLOAT_PRECISION);
            //FIXME: currently fails
            final String moneyFormat = "\"$\"#,##0.00_);[Red]\\(\"$\"#,##0.00\\)";
            assertEquals("[Data Format] O7 data format",
                    moneyFormat, cell.getCellStyle().getDataFormatString());
    ...
    public void testCopyMultipleRows() throws IOException {
        ...
        try (XSSFWorkbook wb = XSSFTestDataSamples.openSampleWorkbook("XSSFSheet.copyRows.xlsx")) {
            ...
            col++;
            cell = CellUtil.getCell(destRow1, col);
            assertEquals("[Style] B10 cell value",
                    "Red", cell.getStringCellValue());
            assertEquals("[Style] B10 cell style",
                    CellUtil.getCell(srcRow1, 1).getCellStyle(), cell.getCellStyle());

            cell = CellUtil.getCell(destRow2, col);
            assertEquals("[Style] B11 cell value",
                    "Blue", cell.getStringCellValue());
            assertEquals("[Style] B11 cell style",
                    CellUtil.getCell(srcRow2, 1).getCellStyle(), cell.getCellStyle());

            // Blank
            col++;
            cell = CellUtil.getCell(destRow1, col);
            assertEquals("[Blank] C10 cell type",
                    CellType.BLANK, cell.getCellType());

            cell = CellUtil.getCell(destRow2, col);
            assertEquals("[Blank] C11 cell type",
                    CellType.BLANK, cell.getCellType());
            ...
            //FIXME: XSSFCell and HSSFCell expose different interfaces. getErrorCellString would be helpful here
            assertEquals("[Error] D10 cell type",
                    CellType.ERROR, cell.getCellType());
            assertEquals("[Error] D10 cell value",
                    FormulaError.NA, error);

            cell = CellUtil.getCell(destRow2, col);
            error = FormulaError.forInt(cell.getErrorCellValue());
            //FIXME: XSSFCell and HSSFCell expose different interfaces. getErrorCellString would be helpful here
            assertEquals("[Error] D11 cell type",
                    CellType.ERROR, cell.getCellType());
            assertEquals("[Error] D11 cell value",
                    FormulaError.NAME, error);

            // Date
            col++;
            cell = CellUtil.getCell(destRow1, col);
            Date date = LocaleUtil.getLocaleCalendar(2000, Calendar.JANUARY, 1).getTime();
            assertEquals("[Date] E10 cell type",
                    CellType.NUMERIC, cell.getCellType());
            assertEquals("[Date] E10 cell value",
                    date, cell.getDateCellValue());

            cell = CellUtil.getCell(destRow2, col);
            date = LocaleUtil.getLocaleCalendar(2000, Calendar.JANUARY, 2).getTime();
            assertEquals("[Date] E11 cell type",
                    CellType.NUMERIC, cell.getCellType());
            assertEquals("[Date] E11 cell value",
                    date, cell.getDateCellValue());

            // Boolean
            col++;
            cell = CellUtil.getCell(destRow1, col);
            assertEquals("[Boolean] F10 cell type",
                    CellType.BOOLEAN, cell.getCellType());
            assertTrue("[Boolean] F10 cell value",
                    cell.getBooleanCellValue());

            cell = CellUtil.getCell(destRow2, col);
            assertEquals("[Boolean] F11 cell type",
                    CellType.BOOLEAN, cell.getCellType());
            assertFalse("[Boolean] F11 cell value",
                    cell.getBooleanCellValue());

            // String
            col++;
            cell = CellUtil.getCell(destRow1, col);
            assertEquals("[String] G10 cell type",
                    CellType.STRING, cell.getCellType());
            assertEquals("[String] G10 cell value",
                    "Hello", cell.getStringCellValue());

            cell = CellUtil.getCell(destRow2, col);
            assertEquals("[String] G11 cell type",
                    CellType.STRING, cell.getCellType());
            assertEquals("[String] G11 cell value",
                    "World", cell.getStringCellValue());

            // Int
            col++;
            cell = CellUtil.getCell(destRow1, col);
            assertEquals("[Int] H10 cell type",
                    CellType.NUMERIC, cell.getCellType());
            assertEquals("[Int] H10 cell value",
                    15, (int) cell.getNumericCellValue());

            cell = CellUtil.getCell(destRow2, col);
            assertEquals("[Int] H11 cell type",
                    CellType.NUMERIC, cell.getCellType());
            assertEquals("[Int] H11 cell value",
                    42, (int) cell.getNumericCellValue());

            // Float
            col++;
            cell = CellUtil.getCell(destRow1, col);
            assertEquals("[Float] I10 cell type",
                    CellType.NUMERIC, cell.getCellType());
            assertEquals("[Float] I10 cell value",
                    12.5, cell.getNumericCellValue(), FLOAT_PRECISION);

            cell = CellUtil.getCell(destRow2, col);
            assertEquals("[Float] I11 cell type",
                    CellType.NUMERIC, cell.getCellType());
            assertEquals("[Float] I11 cell value",
                    5.5, cell.getNumericCellValue(), FLOAT_PRECISION);

            // Cell Formula
            col++;
            cell = CellUtil.getCell(destRow1, col);
            assertEquals("[Cell Formula] J10 cell type",
                    CellType.FORMULA, cell.getCellType());
            assertEquals("[Cell Formula] J10 cell formula",
                    "5+2", cell.getCellFormula());

            cell = CellUtil.getCell(destRow2, col);
            assertEquals("[Cell Formula] J11 cell type",
                    CellType.FORMULA, cell.getCellType());
            assertEquals("[Cell Formula] J11 cell formula",
                    "6+18", cell.getCellFormula());
            ...
            assertEquals("[Cell Formula with Area Reference] M11 cell formula",
                    "SUM($H$3:I10)", cell.getCellFormula());
            ...
            col++;
            cell = CellUtil.getCell(destRow2, col);
            assertEquals("[Data Format] O10 cell type",
                    CellType.NUMERIC, cell.getCellType());
            assertEquals("[Data Format] O10 cell value",
                    100.20, cell.getNumericCellValue(), FLOAT_PRECISION);
            final String moneyFormat = "\"$\"#,##0.00_);[Red]\\(\"$\"#,##0.00\\)";
            assertEquals("[Data Format] O10 cell data format",
                    moneyFormat, cell.getCellStyle().getDataFormatString());
            ...
            cell = CellUtil.getCell(destRow2, col);
            assertEquals("[Merged] P11:Q11 cell value",
                    "Merged cells", cell.getStringCellValue());
            ...
            if (srcRow3 == null) {
                assertNull("Row 3 was empty, so Row 11 should be empty",
                        destRow3);
            }

            // Make sure other rows are blank (off-by-one errors)
            assertNull("Off-by-one lower edge case",
                    sheet.getRow(7)); //one row above destHeaderRow
            assertNull("Off-by-one upper edge case",
                    sheet.getRow(12)); //one row below destRow3
        }
    }
    ...
    public void testRemoveRowWithCommentAndGapAbove() throws IOException {
        try (Workbook wb = _testDataProvider.openSampleWorkbook("59687.xlsx")) {
            ...
            assertEquals("Wrong starting # of comments",
                    1, sheet.getCellComments().size());

            sheet.removeRow(sheet.getRow(commentCellAddress.getRow()));

            assertEquals("There should not be any comments left!",
                    0, sheet.getCellComments().size());
        }
    }
    ...
    public void testNotCascadeWorkbookSetForceFormulaRecalculation() throws IOException {
        try (Workbook wb = _testDataProvider.openSampleWorkbook("sample.xlsx")) {
            ...
            for (Sheet s : wb) {
                assertTrue("Sheet-level flag is still set to true",
                        s.getForceFormulaRecalculation());
            }
        }
    }
    ...
    @Test
    public void testBug64536() throws IOException {
        try (
    ...
    @Test
    public void testNewSheetGetDimension() throws IOException {
    ...
    @Test
    public void testRowShiftWithHyperlink1() throws IOException {
    ...
    @Test
    public void testRowShiftWithHyperlink2() throws IOException {
    ...
    @Test
    public void testRowShiftWithHyperlink3() throws IOException {
        try (XSSFWorkbook wb = createWorkbookForRowShiftWithHyperlink1(false)) {
            XSSFSheet sheet = wb.getSheetAt(0);
            List<XSSFHyperlink> hyperlinks = sheet.getHyperlinkList();
            assertEquals(1, hyperlinks.size());
