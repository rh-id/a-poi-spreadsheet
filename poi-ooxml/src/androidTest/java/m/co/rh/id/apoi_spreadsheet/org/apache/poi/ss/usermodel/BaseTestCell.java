 ...
 * {@link m.co.rh.id.apoi_spreadsheet.org.apache.poi.ss.usermodel.Cell}
 */
...
public abstract class BaseTestCell {
    ...
    public void testConvertStringFormulaCell() throws IOException {
        try (Workbook wb = _testDataProvider.createWorkbook()) {
            ...
            assertFalse("Identified bug with writing back formula result of type string",
                    cellA1.getStringCellValue().isEmpty());
            assertEquals("abc", cellA1.getStringCellValue());
        }
    }
    ...
    public void testNanAndInfinity() throws Exception {
        try (Workbook wb1 = _testDataProvider.createWorkbook()) {
            ...
            assertEquals("Double.POSITIVE_INFINITY should change cell type to CellType#ERROR",
                    CellType.ERROR, cell1.getCellType());
            assertEquals("Double.POSITIVE_INFINITY should change cell value to #DIV/0!",
                    FormulaError.DIV0, forInt(cell1.getErrorCellValue()));

            Cell cell2 = row.createCell(2);
            cell2.setCellValue(Double.NEGATIVE_INFINITY);
            assertEquals("Double.NEGATIVE_INFINITY should change cell type to CellType#ERROR",
                    CellType.ERROR, cell2.getCellType());
            assertEquals("Double.NEGATIVE_INFINITY should change cell value to #DIV/0!",
                    FormulaError.DIV0, forInt(cell2.getErrorCellValue()));
    ...
    @Test
    public void setCellFormulaWithDateArithmetic() throws IOException {
        try (Workbook wb = _testDataProvider.createWorkbook()) {
            ...
            assertEquals(Double.valueOf(DateUtil.getExcelDate(date.plusDays(1))), Double.valueOf(valueA1.getNumberValue()));
        }
    }
